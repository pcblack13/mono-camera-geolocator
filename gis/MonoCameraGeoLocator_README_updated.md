...
The application will support three complementary imagery workflows:

1. Normal online map navigation with read-through caching.
2. Automatic Viewport Caching and Smart Prefetch while the surveyor works.
3. Manual AOI-based Offline Area Download for deliberate field preparation.
...
# 12. Automatic Viewport Caching & Smart Prefetch

MonoCameraGeoLocator will also automatically cache the area where the surveyor is actively working on the Mapbox Satellite map.

This is different from the Manual Offline Area Manager:

- **Automatic Viewport Caching** continuously caches the current working area in the background.
- **Manual Offline Area Download** deliberately prepares a complete AOI for a planned field mission.

Both mechanisms use the **same persistent tile cache**.

## Automatic viewport workflow

```text
User opens Mapbox Satellite
          |
          v
User pans / zooms
          |
...
Automatic Cache Manager
          ...
          |                         |
          v                         v
   Current viewport          Prefetch buffer
          |                         |
          +------------+------------+
                       |
                       v
...
The user does not need to press a download button for normal navigation.

## Smart prefetch

The automatic cache can prefetch an area surrounding the current viewport.
...
This allows the application to cache imagery slightly ahead of the user's movement. The prefetch buffer is configurable, with a conservative default of approximately one surrounding viewport.

## Current zoom by default

Automatic viewport caching normally caches the **current zoom level only**. It does not automatically download every zoom level. This prevents normal navigation from generating unnecessarily large downloads.

## Priority and duplicate protection

Current visible tiles have the highest priority, followed by nearby prefetch work. When the user moves to a new area, obsolete low-priority prefetch work can be cancelled or deprioritized. A tile that is already cached or already in-flight is not downloaded twice.

```text
Tile request
   |
   +-- cached? ---- YES ---> cache hit
   |
   +-- in-flight? - YES ---> reuse existing task
   |
   +-- otherwise ----------> queue one download
```

## Background operation

Automatic caching runs in the background and must not block map panning, zooming, landmark selection, GCP selection, image workflows, or project operations.

## Automatic caching limits

Automatic caching uses bounded resources and respects the existing tile limits, concurrency limits, storage limits, provider rate limits, cache TTL, and eviction policy. It does not create a second tile cache or a second Mapbox client.

## Online/offline behavior

When online, missing viewport tiles can be fetched through the existing Mapbox provider and stored in the persistent cache. When offline, only locally cached tiles are served; missing tiles do not trigger Mapbox network requests.

## Cache status

The map interface can expose status such as:

```text
+----------------------------------+
| Offline Cache                    |
|                                  |
| Automatic caching: ON            |
| Current zoom: 18                 |
| Current area: 94% cached         |
| Cached tiles: 1,248              |
| Missing tiles: 80                |
| Storage: 412 MB                  |
+----------------------------------+
...
## Automatic caching vs. Manual Offline Area Download

**Automatic Viewport Caching** is best for normal navigation, exploratory work, and continuously building a useful local cache around the active work area.

**Manual Offline Area Download** is best for planned field missions where the surveyor wants to select a specific AOI and zoom range and verify complete coverage before leaving Internet coverage.

Both ultimately populate the same persistent tile cache.

---

# 13. Offline Area Manager
...
- **smart viewport prefetching**
- **background tile download queue**
- **current-viewport cache coverage**
...
Automatic Viewport Caching adds a continuous background caching layer: the surveyor can simply work on the map while MonoCameraGeoLocator builds the local cache around the active working area.
...
6. Automatic Viewport Caching caches the active working area

7. Nearby imagery is prefetched in the background
